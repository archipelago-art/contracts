...
describe("ArtblocksRoyaltyOracle", () => {
  let ArtblocksRoyaltyOracle;
  let TestArtblocksRoyaltyDataSource;

  // NimBuds (project 10) royalty data as of mainnet block 14354182.
  const ARTBLOCKS_ADDRESS = "0xf7A55108A6E830a809e88e74cbf5f5DE9D930153";
  const ARTIST_0 = "0x1e8E749b2B578E181Ca01962e9448006772b24a2";
  const ARTIST_1 = "0x4944303979b8d8DA50CcE0f1C145c715950a7abe";
  const ARTIST_1_SPLIT = 40;

  const MICROS_ARTBLOCKS = 25000;
  const MICROS_ARTISTS = 50000;
  const MICROS_ARTIST_0 = (MICROS_ARTISTS * 60) / 100;
  const MICROS_ARTIST_1 = (MICROS_ARTISTS * 40) / 100;
  if (
  ...
  let getRoyalties;
  before(async () => {
    [ArtblocksRoyaltyOracle, TestArtblocksRoyaltyDataSource] =
      await Promise.all([
        ethers.getContractFactory("ArtblocksRoyaltyOracle"),
        ethers.getContractFactory("TestArtblocksRoyaltyDataSource"),
      ]);
    artblocksRoyaltyOracle = await ArtblocksRoyaltyOracle.deploy();
    ...
    // across test cases.
    const testDataSource = await TestArtblocksRoyaltyDataSource.deploy();
  ...
  it("ignores platform royalties if `artblocksAddress()` reverts", async () => {
    expect(await getRoyalties({ artblocksAddressReverts: true })).to.deep.equal(
      [
        [ARTIST_0, MICROS_ARTIST_0],
        [ARTIST_1, MICROS_ARTIST_1],
      ]
